...
const verifyMobile = (mobile, mobileCode) => mdapi.callActionflow({ actionflow_name: "系统_手机验证码校验", payload: { mobile, mobileCode } });
const {
	...
	...(invitationCodeRes ? {
		user_logs: {
			data: [{
				type: "用户邀请",
				title: "注册邀请",
				content: `用户通过邀请码"${invitationCode}"完成注册`,
...
if (!isRegisterMobileRequired) return incUser(userInfo);
	...
	// 手机号和验证码校验
	const mobileVdtRes = verifyMobile(mobile, mobileCode);
