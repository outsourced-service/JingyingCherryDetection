	...
	<div class="system-user-dialog-container">
		<el-dialog v-model="dialog.isShowDialog" destroy-on-close append-to-body fullscreen>
			...
			<div class="layout-container" v-else>
				<userAssets :user_id="dialog.row.id" @closeDialog="closeDialog" style="width: 100%; height: calc(100vh - 32px); top: 32px" />
			</div>
		</el-dialog>
	</div>
</template>

<script setup lang="ts" name="popusRecord">
...
const userAssets = defineAsyncComponent(() => import('./assets/index.vue'));
...
// 打开弹窗
const openDialog = (row: any, open: any) => {
	dialog.row = row;
	dialog.type = open;
